...
public class CPHInline
{
    ...
    public class ModerationConfig
    {
        ...
        public bool ManualReviewMode { get; set; } = false;
        public List<string> AdditionalRegexFilters { get; set; } = new List<string>();
    ...
    {
        public string MsgVoucherBan { get; set; } = "";
        ...
        public string MsgIsBlocked { get; set; } = "";
        public string MsgIsBlockedBan { get; set; } = "";
        public string MsgAutoDenyBan { get; set; } = "";
        ...
        public string MsgAccountBan { get; set; } = "";
    }
    ...
    {
        // Using Regex to capture mentioned usernames.
        return Regex.Matches(message, @"@(\w+)").Cast<Match>().Select(m => m.Groups[1].Value.ToLowerInvariant()).Distinct().ToList();
    }

    // Checks if a message contains diacritical marks
    ...
    private List<string> GetMatchedKeywords(string input, List<string> keywords)
    {
        ...
        foreach (var customPattern in Config.AdditionalRegexFilters)
        {
            try
            {
                Regex customRegex = new Regex(customPattern, RegexOptions.IgnoreCase | RegexOptions.Compiled);
                if (customRegex.IsMatch(inputWithoutUrls) && !matchedKeywords.Contains(customPattern))
                    matchedKeywords.Add(customPattern);
            }
            catch (Exception ex)
            {
                if (Config.EnableDebugLogs)
                    CPH.LogDebug($"[TLG LOG]: Error in regex filter '{customPattern}': {ex.Message}");
            }
        }
    ...
    private bool IsObfuscatedMessage(string message, List<string> suspiciousEndings)
    {
        ...
        var voucherMatches = VoucherCodeRegex.Matches(message);
        if (voucherMatches.Count > 0)
        {
            foreach (var voucherMatch in voucherMatches.Cast<Match>())
            {
                string voucherCode = voucherMatch.Value;
                CPH.LogInfo($"[TLG LOG]: Voucher code detected: '{voucherCode}'");
                return true;
            }
        }
    ...
    // Decides based on Config.sendAction whether to call CPH.SendAction or CPH.SendMessage.
  ...
  ""_comment_ManualReviewMode"": ""If set to true, AutoMod messages are marked for manual review instead of being immediately banned."",
  ""UseTimeout"": false,
  ...
  ""_comment_RequiredKeywordCount"": ""Minimum number of matching keywords required to trigger a moderation action."",
  ...
  ""_comment_MsgVoucherBan"": ""Message displayed when a voucher code is detected and a ban/timeout is executed."",
  ...
  ""_comment_MsgMentionTimeOut"": ""Message used when a timeout is issued due to unauthorized mentions.""
    ...
    public bool Execute()
    {
        ...
        string msgVoucherBan = Messages.MsgVoucherBan.Replace("{user}", user);
        ...
        string msgIsBlocked = Messages.MsgIsBlocked.Replace("{user}", user);
        string msgIsBlockedBan = Messages.MsgIsBlockedBan.Replace("{user}", user);
        string msgAutoDenyBan = Messages.MsgAutoDenyBan.Replace("{user}", user);
        ...
        string msgAccountBan = Messages.MsgAccountBan.Replace("{user}", user);
        //----------------------------------------------------------------
        // 8) Voucher code check
        //----------------------------------------------------------------
        var voucherMatches = VoucherCodeRegex.Matches(input);
        if (voucherMatches.Count > 0)
        {
            foreach (var match in voucherMatches.Cast<Match>())
            {
                string voucherCode = match.Value;
                if (hasEnoughKeywords)
                {
                    string voucherReason = $"Voucher code detected. Message: \"{input}\". Keywords found: [{string.Join(", ", matchedKeywords)}]. TLD: {(globalEndingCheck ?? "None")}";
                    if (Config.UseTimeout)
                    {
                        SendMessageSafe(msgTimeOut, useBot, true);
                        CPH.TwitchTimeoutUser(user, Config.TimeoutDuration, voucherReason, useBot);
                        CPH.LogInfo($"[TLG LOG]: Timeout for '{user}' => Voucher + sufficient keywords. Reason: {voucherReason}");
                    }
                    else
                    {
                        SendMessageSafe(msgVoucherBan, useBot, true);
                        CPH.TwitchBanUser(user, voucherReason, useBot);
                        CPH.LogInfo($"[TLG LOG]: Ban for '{user}' => Voucher + sufficient keywords. Reason: {voucherReason}");
                    }
                }
                else
                {
                    if (Config.UseTwitchWarn)
                    {
                        ...
                        CPH.LogInfo($"[TLG LOG]: Warning for '{user}' => Voucher code detected \"{input}\", but not enough keywords.");
                    }
                }
        ...
        //----------------------------------------------------------------
        // 9) Check for Twitch-blocked (***)
        //----------------------------------------------------------------
        if (IsBlockedByTwitch(input))
        {
            if (hasEnoughKeywords)
            {
                string blockedBanReason = $"Possible spam detected: \"{input}\". Keywords found: [{string.Join(", ", matchedKeywords)}]. TLD: {(globalEndingCheck ?? "None")}";
                if (Config.UseTimeout)
                {
                    SendMessageSafe(msgTimeOut, useBot, true);
                    CPH.TwitchTimeoutUser(user, Config.TimeoutDuration, blockedBanReason, useBot);
                    CPH.LogInfo($"[TLG LOG]: Timeout for '{user}' => *** + sufficient keywords. Reason: {blockedBanReason}");
                }
                else
                {
                    SendMessageSafe(msgUrlBan, useBot, true);
                    CPH.TwitchBanUser(user, blockedBanReason, useBot);
                    CPH.LogInfo($"[TLG LOG]: Ban for '{user}' => *** + sufficient keywords. Reason: {blockedBanReason}");
                }
            ...
            else
            {
                if (Config.UseTwitchWarn)
                {
                    if (!Config.DisableAllMessages)
                        CPH.TwitchWarnUser(user, msgTwitchWarn);
                    CPH.LogInfo($"[TLG LOG]: Warning for '{user}' => *** but not enough keywords.");
                }
            ...
            foreach (var m in urlMatches.Cast<Match>())
            {
                ...
                else
                {
                    if (!string.IsNullOrEmpty(globalEndingCheck))
                    {
                        ...
                        CPH.TwitchDeleteChatMessage(msgId, useBot);
                        CPH.LogInfo($"[TLG LOG]: Message deleted from '{user}' => URL: '{url}'");
                        if (Config.UseTwitchWarn)
                        {
                            if (!Config.DisableAllMessages)
                                CPH.TwitchWarnUser(user, msgTwitchWarn);
        ...
        //----------------------------------------------------------------
        // 12) Check for obfuscated message content
        //----------------------------------------------------------------
        if (globalObfuscated)
        {
            if (hasEnoughKeywords)
            ...
            else
            {
                if (!string.IsNullOrEmpty(globalEndingCheck))
                {
                    ...
                    CPH.LogInfo($"[TLG LOG]: Message deleted from '{user}' => Obfuscated URL: {input}");
                }
        ...
        //----------------------------------------------------------------
        // 13) Handling of AutoMod messages
        //----------------------------------------------------------------
        if (!string.IsNullOrEmpty(msgId) && CPH.TwitchApproveAutoHeldMessage(msgId))
        {
            try
            {
                string denyAutoReason = $"Possible spam detected: \"{input}\". Keywords found: [{string.Join(", ", matchedKeywords)}]. TLD: {(globalEndingCheck ?? "None")}";
                var mentionedUsers = ExtractMentionedUsers(input);
                if (mentionedUsers.Count > 0)
                {
                    if (!string.IsNullOrEmpty(msgAutoDenyBan))
                        SendMessageSafe(msgAutoDenyBan, useBot, true);
                    if (Config.UseTimeout)
                    {
                        CPH.TwitchTimeoutUser(user, Config.TimeoutDuration, denyAutoReason, useBot);
                        CPH.LogInfo($"[TLG LOG]: Timeout for '{user}' => AutoMod message + {matchedKeywordCount} keywords. Reason: {denyAutoReason}");
                    }
                    else
                    {
                        CPH.TwitchBanUser(user, denyAutoReason, useBot);
                        CPH.LogInfo($"[TLG LOG]: Ban for '{user}' => AutoMod message + {matchedKeywordCount} keywords. Reason: {denyAutoReason}");
                    }
                }

                if (Config.ManualReviewMode)
                {
                    CPH.LogInfo($"[TLG LOG]: AutoMod message from '{user}' marked for manual review.");
                }
                else
                {
                    if (!CPH.TwitchDenyAutoHeldMessage(msgId))
                    {
                        CPH.LogError($"[TLG LOG]: Error denying AutoMod message with ID: {msgId}");
                        return true;
                    }
                    ...
                    else if (!string.IsNullOrEmpty(globalEndingCheck))
                    {
                        ...
                        if (Config.UseTwitchWarn)
                        {
                            if (!Config.DisableAllMessages)
                                CPH.TwitchWarnUser(user, msgTwitchWarn);
                            CPH.LogInfo($"[TLG LOG]: Warning for '{user}' => suspicious TLD '{globalEndingCheck}' but not enough keywords.");
                        }

                        CPH.LogInfo($"[TLG LOG]: AutoMod message denied and deleted due to TLD '{globalEndingCheck}' without sufficient keywords.");
                    }
                    else
                    {
                        CPH.LogInfo($"[TLG LOG]: AutoMod message denied and deleted. No action (insufficient keywords/TLD).");
                    }
                }

                string category = args.ContainsKey("category") ? args["category"].ToString() : "Unbekannte Kategorie";
                string heldAt = args.ContainsKey("heldAt") ? args["heldAt"].ToString() : "Unbekannte Zeit";
                CPH.LogInfo($"[TLG LOG]: AutoMod denied => User '{user}', Message='{input}', Category='{category}', Keywords={matchedKeywordCount}, TLD='{globalEndingCheck}'");
            }
            catch (Exception ex)
            {
                CPH.LogError($"[TLG LOG]: Error handling AutoMod message for '{user}': {ex.Message}");
            }
